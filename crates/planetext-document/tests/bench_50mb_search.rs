...
fn bench_search_50mb() {
    ...
    let total_lines = 1_000_000;
    {
        ...
        for i in 0..total_lines {
            if i == target_line {
                writer
                    .write_all(b"line target with key cccquick embedded here\n")
                    .unwrap();
            } else if i % 100_000 == 0 {
                ...
                writer
                    .write_all(b"regular benchmark content line for fifty megabytes text test\n")
                    .unwrap();
            }
        }
        writer.flush().unwrap();
    ...
    println!(
        "[BENCH 50MB] Scan completed in {:?}, total lines: {scanned_lines}",
    ...
    let _read_cached = app
        .read_lines(handle, hit_line_cached, 1)
        .expect("read_lines failed");
    let cached_duration = t_cached_start.elapsed();
    println!(
        "[BENCH 50MB] 2nd search pipeline (cached search + read_lines): {:?}, hits found: {}",
        cached_duration,
        page_cached.hits.len()
    );
    ...
    let _read_ci = app
        .read_lines(handle, hit_line_ci, 1)
        .expect("read_lines failed");
    let ci_duration = t_ci_start.elapsed();
    println!(
        "[BENCH 50MB] Case-insensitive pipeline (search + read_lines): {:?}, hits found: {}",
        ci_duration,
        page_ci.hits.len()
    );
    ...
    let _read_prev = app
        .read_lines(handle, hit_prev.line, 1)
        .expect("read_lines failed");
