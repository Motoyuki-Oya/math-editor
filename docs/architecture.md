...
**文書の本体はGUIフレームワーク接続コードではなく、ネイティブ側のアプリケーションにある。** 本体はファイルの
...
文書の形は**ピースツリー**とする。各ノードは行数とバイト長だけを持ち、葉は
...
- 置換も流す。編集量がメモリ予算を超える場合は、編集内容をテンポラリの
  **世代ファイル**へ流し書きし、ピースツリーはそこを参照する。「すべて置換」が
  ファイル全体に及んでもメモリ使用は一定に保つ。
- Undo は不変な元ファイルと世代ファイルへの参照で表し、削除行の実体をメモリへ
  貯めない。世代ファイルは文書を閉じたときに掃除する。

### 操作ログ(revision)

編集は revision 番号付きの追記専用ログ(行範囲置き換え)として記録し、基本は
メモリに持つ。**ログが単一の真実であり、Undo・下書き・検索インデックスの差分・
スライス整合はすべてこのログ 1 本から導出する。**

- Undo / Redo はこのログから導出する。
...
- それを超えるファイルは、**(バイグラム, 512KB ブロック番号, カウント)** の索引を
  使う。検索語をバイグラムに分解して候補ブロックを絞り、候補だけを seek して
  実走査で確定する。ブロック境界は検索語長ぶんの重なりを付けて走査する。
...
- 検索時は「ベース + 差分」のカウント合算が正のブロックだけを候補にする。
  誤検出は実走査で落ちるが、見逃しは構造的に起きない。ヒット件数の見積りも
  カウント合算から即座に出せ、確定は走査で行う。
...
MVC の原則どおり、1 つの Model に複数の View/Controller が付く。モデルは 2 層に
...
- **スライスモデル**: ビューごとに 1 つ。今見ている範囲(viewport 周辺の行)を
  ...
  カーソル・選択・IME などの編集状態を持つ。文書の真実は持たず、写しはいつでも
...
```mermaid
flowchart TB
    File[ファイルモデル: 1ファイルに1つ<br/>内容・操作ログ・dirty・保存]
    ...
    S1 -- 行範囲置き換え --> File
    S2 -- 行範囲置き換え --> File
    File -- 置き換え通知+revision --> S1
    File -- 置き換え通知+revision --> S2
    S1 -- AST --> V1
    S2 -- AST --> V2
```

- 編集はまずスライスへ同期的に適用され(手元は待たない)、「行範囲置き換え」として
  ファイルモデルへ送られる。ファイルモデルが適用して revision を進め、登録された
  全スライスへ「行範囲置き換え + revision」を通知する。
- 各スライスは自分の窓に掛かる分だけ適用し、掛からなければ行番号の写像だけ行う。
  この写像は文書ストアの操作ログと同じ仕組みで、検索結果とスライスは同じログで
  整合を取る。
...
- 連動する 2 つのスライスが同じファイルを見ていても、各編集は独立の「行範囲
  置き換え」としてファイルモデルで直列化され、revision 写像で全スライスの
  カーソル位置が補正される。二重適用も座標ずれも構造的に起きない。
...
- **Undo も配る入力**: 連動中の Undo/Redo は他の入力と同じ経路で全メンバーへ
  配り、各ファイルモデルが自分のログを 1 ステップ戻す。文書をまたぐ合成履歴は
  作らず、履歴は文書ごとに独立のままとする。連動解除後はフォーカス中の文書だけが
  Undo される。
...
- group(伸びる括弧)は `(x+1)/` + スペースの変換時に作られる。これも深さに
  よらず同じ。
...
ペインごとに保つ。通常クリックまたはEscapeで連動を解除する。同じ文書を
複数ペインで表示する場合は文書本体を共有し、表示状態だけをペインごとに持つ。
