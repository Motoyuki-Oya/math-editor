...
8. [x] **revision付き公開契約と検索スナップショット**: Document Model からの編集は
...
9. [x] **遅延一括操作**: 全行マルチカーソルや「すべて置換」を、対象位置や結果を無制限に
   展開せず、基準 revision 上で再現可能な一括操作として記録する。表示、検索、保存は
   読んだ窓またはブロックへ操作を適用する。
10. [x] **bigram検索索引**: 論理インデックスブロック、可変サイズ、肥大時の実走査単位分割、
    固定ベース索引を実装する。編集差分は独立ログにせず OperationLog から再構築可能な
    キャッシュとし、候補ブロックの実走査で一致を確定する。
11. [x] **メモリ予算・下書き・巨大コピー**: 編集実体、検索結果、索引差分を同じメモリ予算で
