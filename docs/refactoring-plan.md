...
7. **OperationLog + revision**: `Step` の Undo/Redo スタックを、基準 revision と適用位置を
   持つ1本の操作ログへ置き換える。単一・複数・圧縮一括編集、Redo枝、dirty判定、
   古い基準 revision からの座標写像を同じログで扱う。
8. **revision付き公開契約と検索スナップショット**: Document Model からの編集は
   revision 付きの行範囲置換、検索結果等の位置は行・列、内部ログはバイト座標とし、
   Document 境界で変換する。読み取り応答、編集結果、検索スナップショットへ revision
   を付け、古い検索結果を現在座標へ写像し、編集範囲に
   掛かった結果を無効化する。Phase 4 はこの契約をそのまま利用する。
9. **遅延一括操作**: 全行マルチカーソルや「すべて置換」を、対象位置や結果を無制限に
   展開せず、基準 revision 上で再現可能な一括操作として記録する。表示、検索、保存は
   読んだ窓またはブロックへ操作を適用する。
10. **bigram検索索引**: 論理インデックスブロック、可変サイズ、肥大時の実走査単位分割、
    固定ベース索引を実装する。編集差分は独立ログにせず OperationLog から再構築可能な
    キャッシュとし、候補ブロックの実走査で一致を確定する。
11. **メモリ予算・下書き・巨大コピー**: 編集実体、検索結果、索引差分を同じメモリ予算で
    管理し、超過分を操作ログの永続領域へ流す。下書きを「ベース参照 + 操作ログ +
    編集実体 + 育てた索引」へ置き換え、巨大コピーを revision 付き参照にする。
12. **保存と復旧**: 小さいファイルは保存後 Undo 用スナップショットを保持する。
    巨大ファイルはモーダルな進行表示で対象文書への変更操作を停止し、最初の変更位置
    以降の suffix 退避、保存ジャーナル、差分保存、失敗・クラッシュ復旧を実装する。
    成功後だけ新しいベースへ切り替え、操作ログと差分を破棄する。
13. **旧実装の除去と統合検証**: `Fresh(Vec<String>)`、`Step`、全文下書き、標本だけの
    件数推定、全文を組み立てる巨大コピー経路を削除する。小さい文書と巨大文書が同じ
    公開契約を通り、Phase 4 が内部モジュールを知らずに利用できることを確認する。
...
- [ ] 公開界面の行範囲と行・列位置が Document 境界で内部バイト座標へ変換され、
      位置解決にファイル全体の走査を必要としない
- [ ] 検索索引差分が独立した編集ログを持たず、OperationLog から再構築できる
- [ ] 巨大ファイル保存中は対象文書への変更操作が停止し、成功または復旧の完了前に
      操作ログを破棄していない
- [ ] 不正なバイト列を表示用文字列から再構成せず、未編集部分をそのまま保存できる
- [ ] 全行マルチカーソル等の規則的な大量編集を、位置の無制限な列へ展開していない
