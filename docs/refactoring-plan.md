...
8. [ ] **revision付き公開契約と検索スナップショット**: 文書エンジン内部の revision 契約と
   検索結果の座標写像を実装済み。Tauri コマンド境界および Document Model との完全接続は
   Phase 4 で実施する。
9. [ ] **遅延一括操作**: 文書エンジン内部で BulkRule による遅延置換とログ記録を実装済み。
   UI からの巨大ファイル全置換配線と下書きシリアライズは Phase 4 以降で実施する。
10. [ ] **bigram検索索引**: 512KB ブロック bigram 索引と件数推定を実装済み。
    候補ブロックの実走査絞り込みへの完全統合は Phase 4 以降で実施する。
11. [ ] **メモリ予算・下書き・巨大コピー**: 元ファイル参照＋差分ログによる超軽量下書きと
    10MB 巨大コピー制限を実装済み。超過分の自動スピル機構は Phase 5 で実施する。
12. [ ] **保存と復旧**: 一時ファイル書き出しと原子的入れ替えによる安全保存を実装済み。
    巨大ファイルの差分保存と suffix ジャーナル復旧は Phase 5 で実施する。
13. [ ] **旧実装の除去と統合検証**: Phase 3 の基盤（PieceTree, OperationLog, 超軽量下書き,
    Undo/Redoタイムトラベル復元, 800MB巨大ファイル走査）の統合検証とベンチマークを完了。
    Phase 4/5 完了に合わせて全体の最終確認を行う。
