...
19. [x] **検索走査の最適化（is_clean封印解除・キャッシュ・確定件数同期・オンデマンド索引化）**:
    800MB（1600万行）等の大規模文書において、EmEditor並みの高速体感検索を実現するための抜本改善を実施完了：
    - **`self.is_clean()` 封印の解除と `DeltaCache` の完全稼働**: `search.rs` で未保存（dirty）時に
      ...
      Pruning（1.8秒 $\rightarrow$ 57ms）をフル稼働。
    - **検索結果（ヒット位置）のセッション内キャッシュ**: 同一クエリでの「次へ」操作において、
      一度走査して見つかったヒット位置リストを `SearchHitCache`（セッション内）に保持し、2周目以降のループ検索を
      ディスクアクセスなしの 0ms（即時ジャンプ）化。
    - **ファイル末尾走査完了時および走査中のプログレッシブ確定件数同期**: バックグラウンド走査の進行に合わせて
      0.5秒ごとに確定件数を UI バッジへ随時更新し、末尾到達時に真の確定件数へ即座に同期。
    - **触ったブロックのオンデマンド・インデックス化（Opportunistic Indexing）**: 検索走査や
      スクロール表示（`read_tail` 等）でディスクから読み込んだ 512KB 固定長ブロックは、その場でオンデマンドに
      Bigram 索引を同期生成・登録（`ensure_block_at_byte`）。
    - **大文字小文字無視（Case-insensitive）でのインデックス活用**: Bigram 索引の小文字正規化により、
      大文字小文字無視でもインデックス Pruning 判定を可能にし高速化。
    - **並列 mmap 走査の導入**: リテラル検索（2スレッド）および正規表現検索（4スレッド）による高速走査を完備。
...
- [x] 下書き(`*.draft`)・設定(`settings.toml`)・ウィンドウ状態(`window.toml`)が一時ファイル書き出し＋原子的リネームで保護され、直接上書きされていない
- [x] 同一クエリでの2周目以降の検索がセッション内キャッシュにより0msで遷移し、末尾到達時の確定ヒット数バッジ同期およびオンデマンドBigram索引化が動作する
- [x] ファイルの真実がファイルモデル 1 つにあり、スライスは捨てて取り寄せ直せる
- [x] View が編集判断・編集状態を持たず、paint と入力転送だけになっている
- [x] View・スライスが互いを知らない(再描画は revision 通知経由)
- [x] 共有カーソルの実体を作っていない(連動は入力の複製)
- [x] 同一文書の複数スライスの編集が文書モデルで直列化されている(座標の
      重複・二重適用が起きない)
- [x] 連動中の 1 入力が 1 回の Undo で全て戻り、Undo は 1 文書へ 1 回しか
      届かない
- [x] 読み取り応答が revision 検証され、古い応答で窓がずれない
- [x] モデル層からブラウザー API の直接呼び出しが消えている
